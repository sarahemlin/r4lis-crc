...
text_fields <- articles_json %>%
  dplyr::select(c(
    response.docs.abstract, response.docs.snippet, # select() is a _dplyr_ function
...
rank_frequency <- word_counts %>% # take our list of words, minus stop words
  mutate(
    rank = row_number(), # add a new column called rank, which writes the row number for that row
    term_frequency = n / 12731
  ) # add a new column called term frequency, which calculates the frequency for each term by dividing the total number of count of observations in tidy_words dataset (12731) by the count of each word
