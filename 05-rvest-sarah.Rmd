...
## Learning Objective: use _rvest_ to scrape web page content for analysis. {#rvest-los}
...
```{r scrape}
scrape <- read_html("https://www.stlouis-mo.gov/government/departments/aldermen/Wards-1-28.cfm")
...
ward_num <- wards %>%
  ...
  tibble() %>%
  rename(Ward = names(.)) # here the '.' can be read as "the current object, created in the previous line"
```
...
alderman <- wards %>%
  ...
  tibble() %>%
  rename (Alderman = names(.)) # rename the Alderman column, which has a blank header
```
...
```{r ewar-cbind}
wards_alderman <- cbind(ward_num, alderman) # combine the two columns together into one tibble
head(wards_alderman)
```
...
#now we have a table of ward, alderman name, and link to their individual bio page
```
